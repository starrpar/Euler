    ...
    private static void method1(long x) {
        ...
        for (int j = 0; j < candidates.size(); j++) {
            for (long k = 2; k < candidates.get(j); k++) {
                // if candidates[j] is divisible by anything other than 1 an itself, it is not a
                // prime
        ...
        for (long i = 2; i < x; i++) {
            ...
            for (int o = 0; o < candidates.size(); o++) {
                if (i % candidates.get(o) == 0) {
                    // System.out.println(i + " is already divisible by " + candidates.get(o) + ",
                    // the value of o is: " + o);
                    alreadydivisible = true;
                    break;
                }
            }
            // if not found to already be divisible into an existing candidate, test if
            // divides into number
